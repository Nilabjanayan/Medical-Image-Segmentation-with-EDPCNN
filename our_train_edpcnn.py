...
def main(args):
    ...
    total_loss_list = []
    snake_approx_train_loss_list = []
    nll_approx_loss_list = []
    ...
    for epoch in range(1, args.n_epochs + 1):
        for iteration in range(1, int(np.ceil(train.dataset_sz() / args.batch_sz)) + 1):
            ...
            if step % args.log_freq == 0:
                ...
                snake_approx_train_loss_list.append(snake_approx_train_loss)
                nll_approx_loss_list.append(nll_approx_loss)
    ...
    with open('log/edpcnn/total_loss.txt', 'wb') as fp1:
        _pickle.dump(total_loss_list, fp1)
        
    with open('log/edpcnn/snake_approx_train_loss.txt', 'wb') as fp2:
        _pickle.dump(snake_approx_train_loss_list, fp2)
    
    with open('log/edpcnn/nll_approx_loss.txt', 'wb') as fp3:
        _pickle.dump(nll_approx_loss_list, fp3)
        
    with open('log/edpcnn/train_dice.txt', 'wb') as fp4:
        _pickle.dump(train_dice_list, fp4)
        
    with open('log/edpcnn/valid_dice.txt', 'wb') as fp5:
        _pickle.dump(valid_dice_list, fp5)

    return best_val_dice, total_loss_list, snake_approx_train_loss_list, nll_approx_loss_list, train_dice_list, valid_dice_list
